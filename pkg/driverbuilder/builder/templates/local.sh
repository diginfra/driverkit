...
echo "#!/usr/bin/env bash" > "/tmp/diginfra-dkms-make"
echo "make CC={{ .GCCVersion }} \$@" >> "/tmp/diginfra-dkms-make"
chmod +x "/tmp/diginfra-dkms-make"
if [[ -n "${KERNELDIR}" ]]; then
  dkms install --kernelsourcedir ${KERNELDIR} --directive="MAKE='/tmp/diginfra-dkms-make'" -m "{{ .ModuleDriverName }}" -v "{{ .DriverVersion }}" -k "{{ .KernelRelease }}"
else
  dkms install --directive="MAKE='/tmp/diginfra-dkms-make'" -m "{{ .ModuleDriverName }}" -v "{{ .DriverVersion }}" -k "{{ .KernelRelease }}"
fi
rm -Rf "/tmp/diginfra-dkms-make"
{{ else }}
echo "* Building kmod"
{{ if .DownloadSrc }}
